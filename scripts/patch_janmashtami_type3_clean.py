#!/usr/bin/env python3
"""Clean patch: upload Janmashtami Type 3 images, set featured, insert body images, refresh schema."""
...
SLUG = "happy-janmashtami-wishes-quotes-2026"

PROMPTS = json.loads((ROOT / "output/Week1_Rank25_Janmashtami_type3_prompts.json").read_text(encoding="utf-8"))
POST_ID = PROMPTS.get("wp_post_id")

SLOTS = {
    "hero": {
        "src": ROOT / "output/magnific_generated/janmashtami-hero-2026.webp",
        "featured": True,
        "media_title": "happy janmashtami 2026 Hero",
    },
    "flatlay": {
        "src": ROOT / "output/magnific_generated/janmashtami-flatlay-2026.webp",
        "featured": False,
        "media_title": "happy janmashtami 2026 Flatlay",
    },
    "lifestyle": {
        "src": ROOT / "output/magnific_generated/janmashtami-lifestyle-2026.webp",
        "featured": False,
        "media_title": "happy janmashtami 2026 Lifestyle",
    },
}
...
def main():
    if not POST_ID:
        raise SystemExit("Set wp_post_id in Week1_Rank25_Janmashtami_type3_prompts.json first.")
    ...
    for slot, cfg in SLOTS.items():
        ...
        alt = PROMPTS["slots"][slot].get("alt", f"Janmashtami 2026 {slot}")
    ...
    flatlay_target = '<h2 class="wp-block-heading">Janmashtami Quotes</h2>'
    ...
    lifestyle_target = '<h2 class="wp-block-heading">Janmashtami Captions for Instagram</h2>'
    ...
    from publish_janmashtami_article import TITLE, META_DESC, build_faqs

    product_media = json.loads(
        (ROOT / "output/Week1_Rank25_Janmashtami_product_media.json").read_text(encoding="utf-8")
    )
    ...
    blog_posting = {
        ...
        "keywords": [
            "happy janmashtami",
            "janmashtami shubhechha in marathi",
            "janmashtami wishes in english",
            "janmashtami greetings in english",
            "janmashtami quotes",
            "janmashtami wishes 2026",
    ...
    (ROOT / "output/Week1_Rank25_Janmashtami_article.html").write_text(final_content, encoding="utf-8")
    (ROOT / "output/Week1_Rank25_Janmashtami_type3_media.json").write_text(
