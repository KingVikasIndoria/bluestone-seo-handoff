#!/usr/bin/env python3
"""Clean patch: upload Gudi Padwa Meaning Type 3 images, set featured, insert body images, refresh schema."""
...
SLUG = "gudi-padwa-meaning-2027"

PROMPTS = json.loads((ROOT / "output/Week1_Rank26_GudiMeaning_type3_prompts.json").read_text(encoding="utf-8"))
POST_ID = PROMPTS.get("wp_post_id")

SLOTS = {
    "hero": {
        "src": ROOT / "output/magnific_generated/gudi-padwa-meaning-hero-2027.webp",
        "featured": True,
        "media_title": "gudi padwa meaning 2027 Hero",
    },
    "flatlay": {
        "src": ROOT / "output/magnific_generated/gudi-padwa-meaning-flatlay-2027.webp",
        "featured": False,
        "media_title": "gudi padwa meaning 2027 Flatlay",
    },
    "lifestyle": {
        "src": ROOT / "output/magnific_generated/gudi-padwa-meaning-lifestyle-2027.webp",
        "featured": False,
        "media_title": "gudi padwa meaning 2027 Lifestyle",
    },
}
...
def main():
    if not POST_ID:
        raise SystemExit("Set wp_post_id in Week1_Rank26_GudiMeaning_type3_prompts.json first.")
    ...
    flatlay_target = '<h2 class="wp-block-heading">Why Gudi Padwa Is Celebrated</h2>'
    ...
    if flatlay_target not in content_clean:
        raise SystemExit("Could not find Why Gudi Padwa Is Celebrated heading for flatlay insertion.")
    content_clean = content_clean.replace(flatlay_target, f"{flatlay_block}\n\n{flatlay_target}", 1)

    lifestyle_target = '<h2 class="wp-block-heading">How Families Observe Gudi Padwa Rituals</h2>'
    ...
    from publish_gudi_meaning_article import TITLE, META_DESC, build_faqs

    product_media = json.loads(
        (ROOT / "output/Week1_Rank26_GudiMeaning_product_media.json").read_text(encoding="utf-8")
    )
    ...
    (ROOT / "output/Week1_Rank26_GudiMeaning_article.html").write_text(final_content, encoding="utf-8")
    (ROOT / "output/Week1_Rank26_GudiMeaning_type3_media.json").write_text(
