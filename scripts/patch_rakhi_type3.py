#!/usr/bin/env python3
"""Patch all Type 3 images on Raksha Bandhan post WP #14317."""
import base64
import json
import os
import re
...
PROMPTS = json.loads((ROOT / "output/Week1_Rank9_RakshaBandhan_type3_prompts.json").read_text())

SLOTS = {
    "hero": {
        "src": ROOT / "output/magnific_generated/rakhi-hero-2026.webp",
        "featured": True,
    },
    "flatlay": {
        "src": ROOT / "output/magnific_generated/rakhi-flatlay-2026.webp",
        "featured": False,
    },
    "lifestyle": {
        "src": ROOT / "output/magnific_generated/rakhi-lifestyle-2026.webp",
        "featured": False,
    },
}
...
def validate_content(content):
    ...
    rules = {
        ...
        "flatlay_inserted": "rakhi-flatlay-2026.webp" in content,
        "lifestyle_inserted": "rakhi-lifestyle-2026.webp" in content,
    }
...
def main():
    ...
    for slot, cfg in SLOTS.items():
        ...
        alt = PROMPTS["slots"][slot].get("alt", f"Raksha Bandhan 2026 {slot}")
    ...
    flatlay_target = '<!-- wp:heading -->\n<h2 class="wp-block-heading">Emotional Raksha Bandhan Messages</h2>'
    ...
    if flatlay_target not in content_clean:
        flatlay_target_alt = '<h2 class="wp-block-heading">Emotional Raksha Bandhan Messages</h2>'
    ...
    lifestyle_target = '<!-- wp:heading -->\n<h2 class="wp-block-heading">Rakhi Purnima Wishes &amp; Greetings</h2>'
    ...
    if lifestyle_target not in content_clean:
        lifestyle_target_alt = '<h2 class="wp-block-heading">Rakhi Purnima Wishes &amp; Greetings</h2>'
    ...
    product_media_file = ROOT / "output/Week1_Rank9_RakshaBandhan_product_media.json"
    ...
    from optimize_rakhi_article import build_faqs, TITLE, META_DESC, FOCUS_KW, YOAST_TITLE
    ...
    (ROOT / "output/Week1_Rank9_RakshaBandhan_article.html").write_text(final_content)
    print("Saved local HTML file output/Week1_Rank9_RakshaBandhan_article.html")
