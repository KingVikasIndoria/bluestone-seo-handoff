#!/usr/bin/env python3
"""Clean patch: upload Mothers Gift Type 3 images, set featured, insert body images, refresh schema."""
...
SLUG = "gift-for-mothers-day-from-daughter-2027"

PROMPTS = json.loads((ROOT / "output/Week1_Rank28_MothersGift_type3_prompts.json").read_text(encoding="utf-8"))
POST_ID = PROMPTS.get("wp_post_id")

SLOTS = {
    "hero": {
        "src": ROOT / "output/magnific_generated/mothers-day-gift-daughter-hero-2027.webp",
        "featured": True,
        "media_title": "gift for mothers day from daughter 2027 Hero",
    },
    "flatlay": {
        "src": ROOT / "output/magnific_generated/mothers-day-gift-daughter-flatlay-2027.webp",
        "featured": False,
        "media_title": "gift for mothers day from daughter 2027 Flatlay",
    },
    "lifestyle": {
        "src": ROOT / "output/magnific_generated/mothers-day-gift-daughter-lifestyle-2027.webp",
        "featured": False,
        "media_title": "gift for mothers day from daughter 2027 Lifestyle",
    },
}
...
def main():
    if not POST_ID:
        raise SystemExit("Set wp_post_id in Week1_Rank28_MothersGift_type3_prompts.json first.")
    ...
    for slot, cfg in SLOTS.items():
        ...
        alt = PROMPTS["slots"][slot].get("alt", f"Gudi Padwa meaning 2027 {slot}")
    ...
    flatlay_target = '<h2 class="wp-block-heading">Sentimental Keepsakes She Will Treasure</h2>'
    ...
    if flatlay_target not in content_clean:
        raise SystemExit("Could not find Sentimental Keepsakes She Will Treasure heading for flatlay insertion.")
    content_clean = content_clean.replace(flatlay_target, f"{flatlay_block}\n\n{flatlay_target}", 1)

    lifestyle_target = '<h2 class="wp-block-heading">Modern Minimal Gifts for Stylish Moms</h2>'
    ...
    if lifestyle_target not in content_clean:
        raise SystemExit("Could not find rituals heading for lifestyle insertion.")
    content_clean = content_clean.replace(lifestyle_target, f"{lifestyle_block}\n\n{lifestyle_target}", 1)

    import sys

    sys.path.append(str(ROOT / "scripts"))
    from publish_mothers_gift_article import TITLE, META_DESC, build_faqs

    product_media = json.loads(
        (ROOT / "output/Week1_Rank28_MothersGift_product_media.json").read_text(encoding="utf-8")
    )
    ...
    blog_posting = {
        ...
        "keywords": [
            ...
            "new year gift for girlfriend ideas",
            "new year gift for gf",
            "new year gift 2027",
            "new year jewellery gift",
            "romantic jewellery gift",
        ],
    }
    schema_block = (
        "\n\n<!-- wp:html -->\n"
    ...
    (ROOT / "output/Week1_Rank28_MothersGift_article.html").write_text(final_content, encoding="utf-8")
    (ROOT / "output/Week1_Rank28_MothersGift_type3_media.json").write_text(
