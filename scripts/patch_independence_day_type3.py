#!/usr/bin/env python3
"""Patch all Type 3 images on Independence Day post WP #14819."""
import base64
import json
import os
import re
...
PROMPTS = json.loads((ROOT / "output/Week1_Rank6_IndependenceDay_type3_prompts.json").read_text())

SLOTS = {
    "hero": {
        "src": ROOT / "output/magnific_generated/independence-day-hero-2026.webp",
        "featured": True,
    },
    "flatlay": {
        "src": ROOT / "output/magnific_generated/independence-day-flatlay-2026.webp",
        "featured": False,
    },
    "lifestyle": {
        "src": ROOT / "output/magnific_generated/independence-day-lifestyle-2026.webp",
        "featured": False,
    },
}
...
def image_block(mid, src, alt, w, h):
    ...
    return (
        '<!-- wp:image {{"id":{},"sizeSlug":"full","linkDestination":"none"}} -->\n'
        '<figure class="wp-block-image size-full">'
        '<img src="{}" alt="{}" class="wp-image-{}" />'
        '</figure>\n<!-- /wp:image -->'
    ).format(mid, src, clean_alt, mid, w, h)
...
def validate_content(content):
    ...
    rules = {
        ...
        "flatlay_inserted": "independence-day-flatlay-2026.webp" in content,
        "lifestyle_inserted": "independence-day-lifestyle-2026.webp" in content,
    }
...
def main():
    ...
    for slot, cfg in SLOTS.items():
        ...
        alt = PROMPTS["slots"][slot].get("alt", f"Independence Day 2026 {slot}")
    ...
    # Find any <!-- wp:html --> block at the end that contains script tags
    ...
    flatlay_target = '<!-- wp:heading -->\n<h2 class="wp-block-heading">Independence Day Thoughts on Freedom and Responsibility</h2>'
    ...
    if flatlay_target not in content_clean:
        ...
        flatlay_target_alt = '<h2 class="wp-block-heading">Independence Day Thoughts on Freedom and Responsibility</h2>'
    ...
    lifestyle_target = '<!-- wp:heading -->\n<h2 class="wp-block-heading">Thoughtful Independence Day Messages for Cards and Emails</h2>'
    ...
    if lifestyle_target not in content_clean:
        lifestyle_target_alt = '<h2 class="wp-block-heading">Thoughtful Independence Day Messages for Cards and Emails</h2>'
    ...
    # We will read product media list to keep all images
    product_media_file = ROOT / "output/Week1_Rank6_IndependenceDay_product_media.json"
    product_media = json.loads(product_media_file.read_text())
    product_images = [p["src"] for p in product_media]

    # Re-build schemas
    # Re-fetch FAQs from scripts/optimize_independence_day_article.py's build_faqs if we can
    # Let's import build_faqs, etc. from scripts/optimize_independence_day_article.py
    import sys
    sys.path.append(str(ROOT / "scripts"))
    from optimize_independence_day_article import build_faqs, TITLE, META_DESC, FOCUS_KW, YOAST_TITLE
    ...
    (ROOT / "output/Week1_Rank6_IndependenceDay_article.html").write_text(final_content)
    print("Saved local HTML file output/Week1_Rank6_IndependenceDay_article.html")
