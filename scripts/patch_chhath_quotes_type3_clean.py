#!/usr/bin/env python3
"""Clean patch: upload Akshaya Tritiya quotes Type 3 images, set featured, insert body images, refresh schema."""
...
SLUG = "chhath-puja-quotes-in-english-2026"

PROMPTS = json.loads(
    (ROOT / "output/Week1_Rank31_ChhathQuotes_type3_prompts.json").read_text(encoding="utf-8")
)
POST_ID = PROMPTS.get("wp_post_id")

SLOTS = {
    "hero": {
        "src": ROOT / "output/magnific_generated/chhath-puja-hero-2026.webp",
        "featured": True,
        "media_title": "chhath puja quotes in english 2026 Hero",
    },
    "flatlay": {
        "src": ROOT / "output/magnific_generated/chhath-puja-flatlay-2026.webp",
        "featured": False,
        "media_title": "chhath puja quotes in english 2026 Flatlay",
    },
    "lifestyle": {
        "src": ROOT / "output/magnific_generated/chhath-puja-lifestyle-2026.webp",
        "featured": False,
        "media_title": "chhath puja quotes in english 2026 Lifestyle",
    },
}
...
def main():
    if not POST_ID:
        raise SystemExit("Set wp_post_id in Week1_Rank31_ChhathQuotes_type3_prompts.json first.")
    ...
    for slot, cfg in SLOTS.items():
        ...
        alt = PROMPTS["slots"][slot].get("alt", f"Akshaya Tritiya quotes 2027 {slot}")
    ...
    flatlay_target = '<h2 class="wp-block-heading">Chhath Puja Quotes in English</h2>'
    ...
    if flatlay_target not in content_clean:
        raise SystemExit("Could not find Akshaya Tritiya Quotes heading for flatlay insertion.")
    content_clean = content_clean.replace(flatlay_target, f"{flatlay_block}\n\n{flatlay_target}", 1)

    lifestyle_target = '<h2 class="wp-block-heading">Chhath Puja Captions for Instagram</h2>'
    ...
    if lifestyle_target not in content_clean:
        raise SystemExit("Could not find Akshaya Tritiya Captions heading for lifestyle insertion.")
    content_clean = content_clean.replace(lifestyle_target, f"{lifestyle_block}\n\n{lifestyle_target}", 1)

    import sys

    sys.path.append(str(ROOT / "scripts"))
    from publish_chhath_quotes_article import TITLE, META_DESC, build_faqs

    product_media = json.loads(
        (ROOT / "output/Week1_Rank31_ChhathQuotes_product_media.json").read_text(encoding="utf-8")
    )
    ...
    (ROOT / "output/Week1_Rank31_ChhathQuotes_article.html").write_text(final_content, encoding="utf-8")
    (ROOT / "output/Week1_Rank31_ChhathQuotes_type3_media.json").write_text(
