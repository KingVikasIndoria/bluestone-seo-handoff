#!/usr/bin/env python3
"""Publish Week 1 Rank 24: Happy Gudi Padwa in Marathi 2027 (New post)."""
import base64
import json
import os
import re
...
CONTENT_DATA = json.loads(
    (ROOT / "output/Week1_Rank24_GudiPadwa_sections.json").read_text(encoding="utf-8")
)
...
def product_alt(name: str) -> str:
    return f"happy gudi padwa in marathi 2027 gift idea: {name}"
...
def build_carousel(product_media):
    ...
    script = script.replace("bs-cf-eid", "bs-cf-gudipadwa")
    return (
        "<!-- wp:html -->\n<style>\n"
        + style
        + '\n</style>\n<div class="bs-cf" id="bs-cf-gudipadwa" data-interval="3200" aria-roledescription="carousel" aria-label="BlueStone Gudi Padwa gift ideas">\n'
...
def build_faqs():
    html = [h2("Frequently Asked Questions about Happy Gudi Padwa in Marathi")]
    schema = []
...
def build_content(carousel, product_media):
    ...
    parts = [
        ...
        para(
            "Looking for <strong>happy gudi padwa in marathi</strong> you can send on festival morning? "
            "This Gudi Padwa 2027 guide gathers Marathi shubhechha, gudi padwa wishes in english, "
            "gudi padwa greetings in english, and gudi padwa quotes ready to copy for WhatsApp and family chats."
        ),
        para(
            "<strong>TL;DR:</strong> Pick a short Marathi WhatsApp line, a family shubhechha, an English greeting for friends, "
            "or a caption ready quote. Every list is refreshed for Gudi Padwa 2027."
        ),
        para(
            "Gudi Padwa marks the Marathi New Year with a raised Gudi, mango leaves, and blessings for a balanced year ahead. "
            "A sincere wish, sent early, can feel as festive as the ritual when the words sound like your real warmth."
        ),
        h2("Short Happy Gudi Padwa in Marathi for WhatsApp"),
        para(
            "These copy ready happy gudi padwa in marathi lines stay short, warm, and easy to paste into chats and family groups."
        ),
        list_block(SECTIONS["whatsapp_marathi"]),
        h2("Gudi Padwa Shubhechha in Marathi for Family"),
        para(
            "Share these gudi padwa shubhechha in marathi when you want a fuller blessing for parents, elders, and the whole household."
        ),
        list_block(SECTIONS["shubhechha_family"]),
        h2("Gudi Padwa Wishes in English"),
        para(
            "Use these gudi padwa wishes in english for bilingual cards, office chats, and friends outside Maharashtra."
        ),
        list_block(SECTIONS["english_wishes"]),
        h2("Gudi Padwa Greetings in English for Friends"),
        para(
            "These gudi padwa greetings in english keep friendship light, hopeful, and ready for group messages."
        ),
        list_block(SECTIONS["english_greetings_friends"]),
        h2("A Soft Gudi Padwa Gift Idea (If You Are Gifting Too)"),
        para(
            "Gudi Padwa is a beautiful time to mark a fresh year with something lasting. "
            "Earrings, pendants, bangles, rings, and mangalsutra styles make thoughtful keepsakes after the festivities. "
            "Explore these six approved designs from the BlueStone collection."
        ),
        carousel,
        h2("Gudi Padwa Quotes for Cards and Captions"),
        para(
            "Pair these gudi padwa quotes with a puja photo, a raised Gudi reel, or a simple festive status."
        ),
        list_block(SECTIONS["quotes"], ordered=False),
        h2("Traditional and Spiritual Gudi Padwa Wishes"),
        para(
            "Choose these traditional lines when you want a calmer, prayerful tone for elders and morning blessings."
        ),
        list_block(SECTIONS["traditional"]),
        h2("Gudi Padwa Wishes for Colleagues"),
        para("Keep office greetings polite, brief, and hopeful for a productive year ahead."),
        list_block(SECTIONS["colleagues"]),
        h2("How to Pick the Right Gudi Padwa Message"),
        para(
            "Choose a short Marathi WhatsApp line for busy mornings, a family shubhechha for elders, and an English greeting for mixed groups. "
            "Personalize with one name or ritual, keep the tone honest, and refresh older year lines to Gudi Padwa 2027 before you send."
        ),
        h2("More Festive and Occasion Reads"),
        para(
            'Explore our other guides including <a href="https://blog.bluestone.com/happy-ugadi-wishes-in-telugu-2027/">Happy Ugadi wishes in Telugu for 2027</a>, '
        ...
        h2("Conclusion"),
        para(
            "The best happy gudi padwa in marathi lines sound personal, hopeful, and ready to share. "
            "Pick a WhatsApp shubhechha, an English greeting, or a caption quote for Gudi Padwa 2027, "
            "and let your people know the new year begins with warmth. Happy Gudi Padwa 2027!"
        ),
    ]
    content = "\n\n".join(parts)
...
def validate_content(content):
    ...
    rules = {
        ...
        "carousel_mid_article": content.index("bs-cf-gudipadwa") < content.index("Frequently Asked Questions"),
        ...
        "primary_kw": "happy gudi padwa in marathi" in visible.lower(),
...
def main():
    ...
    assets = ROOT / "output/Week1_Rank24_GudiPadwa_assets"
    ...
    (ROOT / "output/Week1_Rank24_GudiPadwa_article.html").write_text(content, encoding="utf-8")
    (ROOT / "output/Week1_Rank24_GudiPadwa_product_media.json").write_text(
    ...
    manifest_path = ROOT / "output/Week1_Rank24_GudiPadwa_type3_prompts.json"
