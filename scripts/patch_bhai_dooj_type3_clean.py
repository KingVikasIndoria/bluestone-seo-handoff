...
POST_ID = 0  # updated after publish
SLUG = "bhai-dooj-wishes"

PROMPTS = json.loads((ROOT / "output/Week1_Rank15_BhaiDooj_type3_prompts.json").read_text())
POST_ID = PROMPTS.get("wp_post_id") or POST_ID

SLOTS = {
    "hero": {
        "src": ROOT / "output/magnific_generated/bhai-dooj-hero-2026.webp",
        "featured": True,
    },
    "flatlay": {
        "src": ROOT / "output/magnific_generated/bhai-dooj-flatlay-2026.webp",
        "featured": False,
    },
    "lifestyle": {
        "src": ROOT / "output/magnific_generated/bhai-dooj-lifestyle-2026.webp",
        "featured": False,
    },
}
...
def main():
    if not POST_ID:
        raise SystemExit("Set wp_post_id in Week1_Rank15_BhaiDooj_type3_prompts.json first.")
    ...
    for slot, cfg in SLOTS.items():
        ...
        alt = PROMPTS["slots"][slot].get("alt", f"Bhai Dooj 2026 {slot}")
    ...
    flatlay_target = '<h2 class="wp-block-heading">Emotional Bhai Dooj Quotes &amp; Bhai Dhooj Quotes</h2>'
    ...
    if flatlay_target in content_clean:
        ...
        raise SystemExit("Could not find quotes heading for flatlay insertion.")

    lifestyle_target = '<h2 class="wp-block-heading">Happy Bhai Dooj Captions for Instagram &amp; Status</h2>'
    ...
    from publish_bhai_dooj_article import TITLE, META_DESC, build_faqs

    product_media = json.loads((ROOT / "output/Week1_Rank15_BhaiDooj_product_media.json").read_text())
    ...
    (ROOT / "output/Week1_Rank15_BhaiDooj_article.html").write_text(final_content)
