#!/usr/bin/env python3
"""Clean patch: upload Best Friend Shayari Type 3 images, set featured, insert body images, refresh schema."""
...
SLUG = "best-friend-shayari-english-2026"

PROMPTS = json.loads(
    (ROOT / "output/Week1_Rank34_BestFriendShayari_type3_prompts.json").read_text(encoding="utf-8")
)
POST_ID = PROMPTS.get("wp_post_id")

SLOTS = {
    "hero": {
        "src": ROOT / "output/magnific_generated/best-friend-shayari-hero-2026.webp",
        "featured": True,
        "media_title": "best friend shayari english 2026 Hero",
    },
    "flatlay": {
        "src": ROOT / "output/magnific_generated/best-friend-shayari-flatlay-2026.webp",
        "featured": False,
        "media_title": "best friend shayari english 2026 Flatlay",
    },
    "lifestyle": {
        "src": ROOT / "output/magnific_generated/best-friend-shayari-lifestyle-2026.webp",
        "featured": False,
        "media_title": "best friend shayari english 2026 Lifestyle",
    },
}
...
def main():
    if not POST_ID:
        raise SystemExit("Set wp_post_id in Week1_Rank34_BestFriendShayari_type3_prompts.json first.")
    ...
    for slot, cfg in SLOTS.items():
        ...
        alt = PROMPTS["slots"][slot].get("alt", f"best friend shayari english 2026 {slot}")
    ...
    from publish_bestfriend_shayari_article import FLATLAY_INSERT_H2, LIFESTYLE_INSERT_H2, META_DESC, TITLE, build_faqs

    flatlay_target = f'<h2 class="wp-block-heading">{FLATLAY_INSERT_H2}</h2>'
    ...
    if flatlay_target not in content_clean:
        raise SystemExit(f"Could not find {FLATLAY_INSERT_H2} heading for flatlay insertion.")
    content_clean = content_clean.replace(flatlay_target, f"{flatlay_block}\n\n{flatlay_target}", 1)

    lifestyle_target = f'<h2 class="wp-block-heading">{LIFESTYLE_INSERT_H2}</h2>'
    ...
    if lifestyle_target not in content_clean:
        raise SystemExit(f"Could not find {LIFESTYLE_INSERT_H2} heading for lifestyle insertion.")
    content_clean = content_clean.replace(lifestyle_target, f"{lifestyle_block}\n\n{lifestyle_target}", 1)

    product_media = json.loads(
        (ROOT / "output/Week1_Rank34_BestFriendShayari_product_media.json").read_text(encoding="utf-8")
    )
    ...
    (ROOT / "output/Week1_Rank34_BestFriendShayari_article.html").write_text(final_content, encoding="utf-8")
    (ROOT / "output/Week1_Rank34_BestFriendShayari_type3_media.json").write_text(
