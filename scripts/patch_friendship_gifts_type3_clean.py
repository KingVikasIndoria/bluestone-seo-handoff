#!/usr/bin/env python3
"""Clean patch: upload Friendship Day gifts Type 3 images, set featured, insert body images, refresh schema."""
...
SLUG = "gifts-for-friends-on-friendship-day-2026"

PROMPTS = json.loads(
    (ROOT / "output/Week1_Rank29_FriendshipGifts_type3_prompts.json").read_text(encoding="utf-8")
)
POST_ID = PROMPTS.get("wp_post_id")

SLOTS = {
    "hero": {
        "src": ROOT / "output/magnific_generated/friendship-day-gifts-hero-2026.webp",
        "featured": True,
        "media_title": "gifts for friends on friendship day 2026 Hero",
    },
    "flatlay": {
        "src": ROOT / "output/magnific_generated/friendship-day-gifts-flatlay-2026.webp",
        "featured": False,
        "media_title": "gifts for friends on friendship day 2026 Flatlay",
    },
    "lifestyle": {
        "src": ROOT / "output/magnific_generated/friendship-day-gifts-lifestyle-2026.webp",
        "featured": False,
        "media_title": "gifts for friends on friendship day 2026 Lifestyle",
    },
}
...
def main():
    if not POST_ID:
        raise SystemExit("Set wp_post_id in Week1_Rank29_FriendshipGifts_type3_prompts.json first.")
    ...
    for slot, cfg in SLOTS.items():
        ...
        alt = PROMPTS["slots"][slot].get("alt", f"Friendship Day gifts 2026 {slot}")
    ...
    flatlay_target = '<h2 class="wp-block-heading">Matching or Duo Gift Ideas</h2>'
    ...
    if flatlay_target not in content_clean:
        raise SystemExit("Could not find Matching or Duo Gift Ideas heading for flatlay insertion.")
    content_clean = content_clean.replace(flatlay_target, f"{flatlay_block}\n\n{flatlay_target}", 1)

    lifestyle_target = '<h2 class="wp-block-heading">How to Choose the Right Friendship Day Gift</h2>'
    ...
    if lifestyle_target not in content_clean:
        raise SystemExit("Could not find How to Choose heading for lifestyle insertion.")
    content_clean = content_clean.replace(lifestyle_target, f"{lifestyle_block}\n\n{lifestyle_target}", 1)

    import sys

    sys.path.append(str(ROOT / "scripts"))
    from publish_friendship_gifts_article import TITLE, META_DESC, build_faqs

    product_media = json.loads(
        (ROOT / "output/Week1_Rank29_FriendshipGifts_product_media.json").read_text(encoding="utf-8")
    )
    ...
    (ROOT / "output/Week1_Rank29_FriendshipGifts_article.html").write_text(final_content, encoding="utf-8")
    (ROOT / "output/Week1_Rank29_FriendshipGifts_type3_media.json").write_text(
