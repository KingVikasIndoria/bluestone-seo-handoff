#!/usr/bin/env python3
"""Clean patch: upload Gudi Padwa Type 3 images, set featured, insert body images, refresh schema."""
...
SLUG = "happy-gudi-padwa-in-marathi-2027"

PROMPTS = json.loads((ROOT / "output/Week1_Rank24_GudiPadwa_type3_prompts.json").read_text(encoding="utf-8"))
POST_ID = PROMPTS.get("wp_post_id")

SLOTS = {
    "hero": {
        "src": ROOT / "output/magnific_generated/gudi-padwa-hero-2027.webp",
        "featured": True,
        "media_title": "happy gudi padwa in marathi 2027 Hero",
    },
    "flatlay": {
        "src": ROOT / "output/magnific_generated/gudi-padwa-flatlay-2027.webp",
        "featured": False,
        "media_title": "happy gudi padwa in marathi 2027 Flatlay",
    },
    "lifestyle": {
        "src": ROOT / "output/magnific_generated/gudi-padwa-lifestyle-2027.webp",
        "featured": False,
        "media_title": "happy gudi padwa in marathi 2027 Lifestyle",
    },
}
...
def main():
    if not POST_ID:
        raise SystemExit("Set wp_post_id in Week1_Rank24_GudiPadwa_type3_prompts.json first.")
    ...
    for slot, cfg in SLOTS.items():
        ...
        alt = PROMPTS["slots"][slot].get("alt", f"Gudi Padwa 2027 {slot}")
    ...
    flatlay_target = '<h2 class="wp-block-heading">Gudi Padwa Wishes in English</h2>'
    ...
    lifestyle_target = '<h2 class="wp-block-heading">Gudi Padwa Quotes for Cards and Captions</h2>'
    ...
    if lifestyle_target not in content_clean:
        raise SystemExit("Could not find quotes heading for lifestyle insertion.")
    content_clean = content_clean.replace(lifestyle_target, f"{lifestyle_block}\n\n{lifestyle_target}", 1)

    import sys

    sys.path.append(str(ROOT / "scripts"))
    from publish_gudi_padwa_article import TITLE, META_DESC, build_faqs

    product_media = json.loads(
        (ROOT / "output/Week1_Rank24_GudiPadwa_product_media.json").read_text(encoding="utf-8")
    )
    ...
    blog_posting = {
        ...
        "keywords": [
            ...
            "gudi padwa greetings in english",
            "gudi padwa quotes",
            "gudi padwa wishes 2027",
            "happy gudi padwa 2027",
        ],
    }
    schema_block = (
        "\n\n<!-- wp:html -->\n"
    ...
    (ROOT / "output/Week1_Rank24_GudiPadwa_article.html").write_text(final_content, encoding="utf-8")
    (ROOT / "output/Week1_Rank24_GudiPadwa_type3_media.json").write_text(
