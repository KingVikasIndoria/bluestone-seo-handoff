#!/usr/bin/env python3
"""Clean patch: upload Mother's Day Type 3 images, set featured, insert body images, refresh schema."""
...
PROMPTS = json.loads((ROOT / "output/Week1_Rank22_MothersDay_type3_prompts.json").read_text(encoding="utf-8"))
POST_ID = PROMPTS.get("wp_post_id")

SLOTS = {
    "hero": {
        "src": ROOT / "output/magnific_generated/mothers-day-hero-2027.webp",
        "featured": True,
        "media_title": "mother's day wishes in english 2027 Hero",
    },
    "flatlay": {
        "src": ROOT / "output/magnific_generated/mothers-day-flatlay-2027.webp",
        "featured": False,
        "media_title": "mother's day wishes in english 2027 Flatlay",
    },
    "lifestyle": {
        "src": ROOT / "output/magnific_generated/mothers-day-lifestyle-2027.webp",
        "featured": False,
        "media_title": "mother's day wishes in english 2027 Lifestyle",
    },
}
...
def main():
    if not POST_ID:
        raise SystemExit("Set wp_post_id in Week1_Rank22_MothersDay_type3_prompts.json first.")
    ...
    for slot, cfg in SLOTS.items():
        ...
        alt = PROMPTS["slots"][slot].get("alt", f"Mother's Day 2027 {slot}")
    ...
    flatlay_candidates = [
        '<h2 class="wp-block-heading">Emotional Mother&#x27;s Day Messages for Cards</h2>',
        '<h2 class="wp-block-heading">Emotional Mother&#8217;s Day Messages for Cards</h2>',
        '<h2 class="wp-block-heading">Emotional Mother\'s Day Messages for Cards</h2>',
        '<h2 class="wp-block-heading">Emotional Mother’s Day Messages for Cards</h2>',
    ]
    ...
    inserted = False
    for target in flatlay_candidates:
        if target in content_clean:
            content_clean = content_clean.replace(target, f"{flatlay_block}\n\n{target}", 1)
            inserted = True
            break
    if not inserted:
        raise SystemExit("Could not find emotional messages heading for flatlay insertion.")

    lifestyle_target = '<h2 class="wp-block-heading">Mothers Day Status and Instagram Captions</h2>'
    ...
    if lifestyle_target in content_clean:
        ...
        raise SystemExit("Could not find status heading for lifestyle insertion.")

    import sys

    sys.path.append(str(ROOT / "scripts"))
    from publish_mothers_day_article import TITLE, META_DESC, build_faqs

    product_media = json.loads(
        (ROOT / "output/Week1_Rank22_MothersDay_product_media.json").read_text(encoding="utf-8")
    )
    ...
    (ROOT / "output/Week1_Rank22_MothersDay_article.html").write_text(final_content, encoding="utf-8")
    (ROOT / "output/Week1_Rank22_MothersDay_type3_media.json").write_text(
