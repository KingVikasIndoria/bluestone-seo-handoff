...
PROMPTS = json.loads((ROOT / "output/Week1_Rank16_ValentinesHubby_type3_prompts.json").read_text())

SLOTS = {
    "hero": {
        "src": ROOT / "output/magnific_generated/valentines-hero-2027.webp",
        "featured": True,
    },
    "flatlay": {
        "src": ROOT / "output/magnific_generated/valentines-flatlay-2027.webp",
        "featured": False,
    },
    "lifestyle": {
        "src": ROOT / "output/magnific_generated/valentines-lifestyle-2027.webp",
        "featured": False,
    },
}
...
def main():
    ...
    for slot, cfg in SLOTS.items():
        ...
        alt = PROMPTS["slots"][slot].get("alt", f"Valentine's Day 2027 {slot}")
    ...
    flatlay_target = '<h2 class="wp-block-heading">Valentine&#x27;s Hubby Quotes &amp; Valentine Quotes for Husband</h2>'
    ...
    if flatlay_target in content_clean:
        ...
        raise SystemExit("Could not find hubby quotes heading for flatlay insertion.")

    lifestyle_target = '<h2 class="wp-block-heading">Valentine&#x27;s Captions for Instagram &amp; Status</h2>'
    ...
    from optimize_valentines_hubby_article import TITLE, META_DESC, build_faqs

    product_media = json.loads((ROOT / "output/Week1_Rank16_ValentinesHubby_product_media.json").read_text())
    ...
    (ROOT / "output/Week1_Rank16_ValentinesHubby_article.html").write_text(final_content)
