#!/usr/bin/env python3
"""Clean patch: upload Republic Day why Type 3 images, set featured, insert body images, refresh schema."""
...
SLUG = "why-republic-day-is-celebrated-2027"

PROMPTS = json.loads(
    (ROOT / "output/Week1_Rank33_RepublicWhy_type3_prompts.json").read_text(encoding="utf-8")
)
POST_ID = PROMPTS.get("wp_post_id")

SLOTS = {
    "hero": {
        "src": ROOT / "output/magnific_generated/republic-why-hero-2027.webp",
        "featured": True,
        "media_title": "why republic day is celebrated 2027 Hero",
    },
    "flatlay": {
        "src": ROOT / "output/magnific_generated/republic-why-flatlay-2027.webp",
        "featured": False,
        "media_title": "why republic day is celebrated 2027 Flatlay",
    },
    "lifestyle": {
        "src": ROOT / "output/magnific_generated/republic-why-lifestyle-2027.webp",
        "featured": False,
        "media_title": "why republic day is celebrated 2027 Lifestyle",
    },
}
...
def main():
    if not POST_ID:
        raise SystemExit("Set wp_post_id in Week1_Rank33_RepublicWhy_type3_prompts.json first.")
    ...
    for slot, cfg in SLOTS.items():
        ...
        alt = PROMPTS["slots"][slot].get("alt", f"why republic day is celebrated 2027 {slot}")
    ...
    from publish_republic_why_article import FLATLAY_INSERT_H2, LIFESTYLE_INSERT_H2, META_DESC, TITLE, build_faqs
    ...
    product_media = json.loads(
        (ROOT / "output/Week1_Rank33_RepublicWhy_product_media.json").read_text(encoding="utf-8")
    )
    ...
    (ROOT / "output/Week1_Rank33_RepublicWhy_article.html").write_text(final_content, encoding="utf-8")
    (ROOT / "output/Week1_Rank33_RepublicWhy_type3_media.json").write_text(
