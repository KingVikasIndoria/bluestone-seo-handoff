#!/usr/bin/env python3
"""Publish Week 1 Rank 29: Friendship Day 2026 gift guide (New post)."""
import base64
import json
import os
import re
...
CONTENT_DATA = json.loads(
    (ROOT / "output/Week1_Rank29_FriendshipGifts_sections.json").read_text(encoding="utf-8")
)
...
INTRO = META.get("intro") or (
    "Looking for thoughtful <strong>gifts for friends on friendship day</strong> for Friendship Day 2026? "
    "This jewellery gift guide gathers warm ideas for best friends, girl friends, colleagues, and matching duos."
)
...
PRODUCTS = [
    ...
    {"code":"BIAR0097R07","name":"The Liza ring","url":"https://www.bluestone.com/rings/the-liza-ring~7623.html","png": ROOT / "ProductImages/seo images/Rings/The Liza ring.png"},
]
...
def product_alt(name: str) -> str:
    return f"gifts for friends on friendship day 2026 gift idea: {name}"
...
def build_carousel(product_media):
    ...
    script = script.replace("bs-cf-eid", "bs-cf-friendgifts")
    return (
        "<!-- wp:html -->\n<style>\n"
        + style
        + '\n</style>\n<div class="bs-cf" id="bs-cf-friendgifts" data-interval="3200" aria-roledescription="carousel" aria-label="BlueStone Friendship Day 2026 gift ideas">\n'
...
def build_faqs():
    html = [h2("Frequently Asked Questions about Gifts for Friends on Friendship Day")]
    schema = []
...
def build_content(carousel, product_media):
    ...
    if intro and "<strong>" not in intro:
        intro = re.sub(r"(?i)gifts\ for\ friends\ on\ friendship\ day", f"<strong>gifts for friends on friendship day</strong>", intro, count=1)
    if not intro:
        intro = f'Looking for a thoughtful <strong>gifts for friends on friendship day</strong> for Friendship Day 2026? This jewellery gift guide gathers warm, practical ideas she will wear long after the day.'
    parts = [
        ...
        h2("Gifts for Best Friends on Friendship Day"),
        para("Choose these when you want gifts for friends on friendship day that feel personal, not generic."),
        list_block(SECTIONS["for_best_friend"]),
        h2("Friendship Day Gifts for Girl Friends"),
        para("Light earrings, pendants, and bracelets make easy friendship day gifts for her everyday outfits."),
        list_block(SECTIONS["for_girl_friends"]),
        h2("Thoughtful Gifts for Colleague Friends"),
        para("Keep office-friendly jewellery polite, wearable, and warm for Friendship Day 2026."),
        list_block(SECTIONS["for_colleagues"]),
        h2("Shop Ready Friendship Day Gift Ideas"),
        para("Browse six approved BlueStone designs when you need concrete gifts for friends on friendship day."),
        carousel,
        h2("Matching or Duo Gift Ideas"),
        para("Pick complementary pieces when you and your friend want a shared Friendship Day memory."),
        list_block(SECTIONS["matching_or_duo"]),
        h2("How to Choose the Right Friendship Day Gift"),
        para("Match her style and your friendship story before you buy."),
        list_block(SECTIONS["how_to_choose"]),
        h2("Gifting Tips for Friendship Day 2026"),
        para("A short note and good timing make jewellery feel like friendship, not a transaction."),
        ...
        faq_html,
        h2("Conclusion"),
        para("The best gifts for friends on friendship day for 2026 feel light, lasting, and true to your bond. Pick best-friend, girl-friend, colleague, or duo jewellery, and celebrate Friendship Day with something she will wear again."),
    ]
    content = "\n\n".join(parts)
    ...
    blog_posting = {
        ...
        "keywords": ["gifts for friends on friendship day", "friendship day gifts for best friend", "friendship day gift ideas", "friendship day 2026"],
    }
    content += (
        "\n\n<!-- wp:html -->\n"
...
def validate_content(content):
    ...
    rules = {
        ...
        "carousel_mid_article": content.index("bs-cf-friendgifts") < content.index("Frequently Asked Questions"),
        ...
        "primary_kw": "gifts for friends on friendship day" in visible.lower(),
...
def main():
    ...
    assets = ROOT / "output/Week1_Rank29_FriendshipGifts_assets"
    ...
    (ROOT / "output/Week1_Rank29_FriendshipGifts_article.html").write_text(content, encoding="utf-8")
    (ROOT / "output/Week1_Rank29_FriendshipGifts_product_media.json").write_text(
    ...
    manifest_path = ROOT / "output/Week1_Rank29_FriendshipGifts_type3_prompts.json"
