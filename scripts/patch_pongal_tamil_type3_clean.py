...
PROMPTS = json.loads((ROOT / "output/Week1_Rank19_PongalTamil_type3_prompts.json").read_text())

SLOTS = {
    "hero": {
        "src": ROOT / "output/magnific_generated/pongal-hero-2027.webp",
        "featured": True,
    },
    "flatlay": {
        "src": ROOT / "output/magnific_generated/pongal-flatlay-2027.webp",
        "featured": False,
    },
    "lifestyle": {
        "src": ROOT / "output/magnific_generated/pongal-lifestyle-2027.webp",
        "featured": False,
    },
}
...
def main():
    ...
    for slot, cfg in SLOTS.items():
        ...
        alt = PROMPTS["slots"][slot].get("alt", f"Pongal 2027 {slot}")
    ...
    flatlay_targets = [
        '<h2 class="wp-block-heading">Pongal Valthukkal Tamil Greetings</h2>',
    ]
    lifestyle_targets = [
        '<h2 class="wp-block-heading">WhatsApp Pongal Wishes in Tamil &amp; Text</h2>',
        '<h2 class="wp-block-heading">WhatsApp Pongal Wishes in Tamil & Text</h2>',
    ]
    ...
    for target in flatlay_targets:
        ...
        raise SystemExit("Could not find valthukkal heading for flatlay insertion.")
    ...
    for target in lifestyle_targets:
        ...
        raise SystemExit("Could not find WhatsApp heading for lifestyle insertion.")

    import sys
    sys.path.append(str(ROOT / "scripts"))
    from optimize_pongal_tamil_article import TITLE, META_DESC, build_faqs

    product_media = json.loads((ROOT / "output/Week1_Rank19_PongalTamil_product_media.json").read_text())
    ...
    (ROOT / "output/Week1_Rank19_PongalTamil_article.html").write_text(final_content)
