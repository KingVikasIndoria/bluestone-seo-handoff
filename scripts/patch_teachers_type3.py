#!/usr/bin/env python3
"""Patch all Type 3 images on Teachers' Day post WP #30030."""
import base64
import json
import os
import re
...
PROMPTS = json.loads((ROOT / "output/Week1_Rank10_TeachersDay_type3_prompts.json").read_text())

SLOTS = {
    "hero": {
        "src": ROOT / "output/magnific_generated/teachers-hero-2026.webp",
        "featured": True,
    },
    "flatlay": {
        "src": ROOT / "output/magnific_generated/teachers-flatlay-2026.webp",
        "featured": False,
    },
    "lifestyle": {
        "src": ROOT / "output/magnific_generated/teachers-lifestyle-2026.webp",
        "featured": False,
    },
}
...
def validate_content(content):
    ...
    rules = {
        ...
        "flatlay_inserted": "teachers-flatlay-2026.webp" in content,
        "lifestyle_inserted": "teachers-lifestyle-2026.webp" in content,
    }
...
def main():
    ...
    for slot, cfg in SLOTS.items():
        ...
        alt = PROMPTS["slots"][slot].get("alt", f"Teachers' Day 2026 {slot}")
    ...
    flatlay_target = '<!-- wp:heading -->\n<h2 class="wp-block-heading">Poetic &amp; Creative Wishes</h2>'
    ...
    if flatlay_target not in content_clean:
        flatlay_target_alt = '<h2 class="wp-block-heading">Poetic &amp; Creative Wishes</h2>'
    ...
    lifestyle_target = '<!-- wp:heading -->\n<h2 class="wp-block-heading">Messages from Parents</h2>'
    ...
    if lifestyle_target not in content_clean:
        lifestyle_target_alt = '<h2 class="wp-block-heading">Messages from Parents</h2>'
    ...
    product_media_file = ROOT / "output/Week1_Rank10_TeachersDay_product_media.json"
    ...
    from publish_teachers_article import build_faqs, TITLE, META_DESC, FOCUS_KW, YOAST_TITLE
    ...
    (ROOT / "output/Week1_Rank10_TeachersDay_article.html").write_text(final_content)
    print("Saved local HTML file output/Week1_Rank10_TeachersDay_article.html")
