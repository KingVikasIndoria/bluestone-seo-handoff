#!/usr/bin/env python3
"""Clean patch: upload New Year Love Type 3 images, set featured, insert body images, refresh schema."""
...
SLUG = "happy-new-year-wishes-for-love-2027"

PROMPTS = json.loads((ROOT / "output/Week1_Rank23_NewYearLove_type3_prompts.json").read_text(encoding="utf-8"))
POST_ID = PROMPTS.get("wp_post_id")

SLOTS = {
    "hero": {
        "src": ROOT / "output/magnific_generated/new-year-love-hero-2027.webp",
        "featured": True,
        "media_title": "happy new year wishes for love 2027 Hero",
    },
    "flatlay": {
        "src": ROOT / "output/magnific_generated/new-year-love-flatlay-2027.webp",
        "featured": False,
        "media_title": "happy new year wishes for love 2027 Flatlay",
    },
    "lifestyle": {
        "src": ROOT / "output/magnific_generated/new-year-love-lifestyle-2027.webp",
        "featured": False,
        "media_title": "happy new year wishes for love 2027 Lifestyle",
    },
}
...
def main():
    if not POST_ID:
        raise SystemExit("Set wp_post_id in Week1_Rank23_NewYearLove_type3_prompts.json first.")
    ...
    for slot, cfg in SLOTS.items():
        ...
        alt = PROMPTS["slots"][slot].get("alt", f"New Year 2027 {slot}")
    ...
    flatlay_target = '<h2 class="wp-block-heading">New Year Wishes for Girlfriend</h2>'
    ...
    if flatlay_target not in content_clean:
        raise SystemExit("Could not find girlfriend heading for flatlay insertion.")
    content_clean = content_clean.replace(flatlay_target, f"{flatlay_block}\n\n{flatlay_target}", 1)

    lifestyle_target = '<h2 class="wp-block-heading">Cute New Year Wishes for Friends</h2>'
    ...
    if lifestyle_target not in content_clean:
        raise SystemExit("Could not find cute friends heading for lifestyle insertion.")
    content_clean = content_clean.replace(lifestyle_target, f"{lifestyle_block}\n\n{lifestyle_target}", 1)

    import sys

    sys.path.append(str(ROOT / "scripts"))
    from publish_new_year_love_article import TITLE, META_DESC, build_faqs

    product_media = json.loads(
        (ROOT / "output/Week1_Rank23_NewYearLove_product_media.json").read_text(encoding="utf-8")
    )
    ...
    (ROOT / "output/Week1_Rank23_NewYearLove_article.html").write_text(final_content, encoding="utf-8")
    (ROOT / "output/Week1_Rank23_NewYearLove_type3_media.json").write_text(
